...
const INTRO_CLOSE_DELAY_MS = 1280;
...
export function Intro({ ctaLabel = "Access portfolio" }: IntroProps) {
  ...
  return (
    ...
    >
      <div
        className={`intro-overlay__backdrop pointer-events-none absolute inset-0 transition-[background-color,opacity,backdrop-filter] duration-[700ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${
          isMainRevealed ? "bg-[rgba(1,1,3,0.66)]" : "bg-[rgba(1,1,3,0.9)]"
        } ${
          isClosing
            ? "intro-overlay__backdrop--closing"
            : isMainRevealed
      ...
      />
      {isClosing ? (
        <div aria-hidden="true" className="intro-overlay__doors absolute inset-0 z-[70]">
          <div className="intro-door intro-door--left intro-door--left-open" />
          <div className="intro-door intro-door--right intro-door--right-open" />
          <div className="intro-door__seam intro-door__seam--active" />
        </div>
      ) : null}
      <div
        aria-hidden="true"
        className={`intro-overlay__beam pointer-events-none absolute inset-y-[-15%] left-[-20%] w-[45%] ${
          isClosing ? "intro-overlay__effect--closing" : "opacity-100"
        }`}
      />
      <div
        aria-hidden="true"
        className={`pointer-events-none absolute left-1/2 top-1/2 h-[72vw] w-[72vw] min-h-[680px] min-w-[680px] -translate-x-1/2 -translate-y-1/2 rounded-full transition-[transform,opacity] duration-[700ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${
          isClosing ? "intro-overlay__effect--closing" : "scale-100 opacity-100"
        }`}
      >
        <div className="intro-overlay__aura h-full w-full rounded-full" />
      </div>
      <div
        aria-hidden="true"
        className={`intro-overlay__ring pointer-events-none absolute left-1/2 top-1/2 h-[46vw] w-[46vw] min-h-[360px] min-w-[360px] rounded-full border border-[rgba(255,90,90,0.18)] ${
          isClosing ? "intro-overlay__effect--closing" : "opacity-100"
        }`}
      />
      <div
        aria-hidden="true"
        className={`intro-overlay__ring intro-overlay__ring--delayed pointer-events-none absolute left-1/2 top-1/2 h-[58vw] w-[58vw] min-h-[460px] min-w-[460px] rounded-full border border-[rgba(255,255,255,0.08)] ${
          isClosing ? "intro-overlay__effect--closing" : "opacity-100"
        }`}
      />
      <div
        aria-hidden="true"
        className={`pointer-events-none absolute inset-x-0 top-1/2 h-px -translate-y-1/2 bg-[linear-gradient(90deg,transparent,rgba(255,91,91,0.55),transparent)] transition-opacity duration-700 ${
          isClosing ? "intro-overlay__effect--closing" : "opacity-100"
        }`}
      />
      ...
      <div
        ...
        } ${isClosing ? "duration-[680ms] scale-[1.01] opacity-0" : ""}`}
        style={isClosing ? { transitionDelay: "180ms" } : undefined}
      >
        <div
          aria-hidden="true"
        ...
        <div
          ...
          } ${isClosing ? "duration-[520ms] scale-[0.99] opacity-0 blur-sm" : ""}`}
          style={isClosing ? { transitionDelay: "90ms" } : undefined}
        >
          <div
